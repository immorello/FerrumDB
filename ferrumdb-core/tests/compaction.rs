...
fn test_compact_merges_into_one() {
    ...
    for i in 0..4 {
        assert_eq!(store.get_value(&format!("key_{}", i)).unwrap(), Some(Value::Integer(i)));
    }
...
fn test_compact_keeps_newest_value() {
    ...
    store.flush().unwrap();
    store.set_value("k".to_string(), Value::Integer(3)).unwrap();
    store.flush().unwrap();

    store.compact().unwrap();

    assert_eq!(count_sstables(&dir), 1);
    assert_eq!(store.get_value("k").unwrap(), Some(Value::Integer(3)), "newest value must win");
...
fn test_compact_drops_deleted_keys() {
    ...
    assert_eq!(count_sstables(&dir), 1);
    assert_eq!(store.get_value("a").unwrap(), None, "deleted key must not survive compaction");
...
fn test_auto_compaction_bounds_sstable_count() {
...

    {
        ...
        for i in 0..20 {
            store.set_value(format!("key_{:02}", i), Value::Integer(i)).unwrap();
            store.flush().unwrap();
        }
    ...
    for i in 0..20 {
        assert_eq!(
            store.get_value(&format!("key_{:02}", i)).unwrap(),
